...
class HypergraphDatasetDGL:
    ...
    def load_data(self):
        ...
        if self.dataset != "BioKG":
            with open(os.path.join(data_path, "phenotype_to_idx.json"), 'r') as f:
                self.d_phenotype_name2id = json.load(f)
        else:
            with open(os.path.join(data_path, "pathway_to_idx.json"), 'r') as f:
                self.d_phenotype_name2id = json.load(f)
        ...
        self.d_phenotype_id2name = {idx: phenotype for phenotype, idx in self.d_phenotype_name2id.items()}

        self.num_drugs = len(self.d_drug_name2id)
        self.num_diseases = len(self.d_disease_name2id)
        self.num_genes = len(self.d_gene_name2id)
        self.num_phenotypes = len(self.d_phenotype_name2id)
        ...
        if self.dataset != "BioKG":
            self.df_drug_phenotypes = pd.read_csv(os.path.join(data_path, "drug-phenotype.csv"))
            self.df_disease_phenotypes = pd.read_csv(os.path.join(data_path, "disease-phenotype.csv"))
        else:
            self.df_drug_phenotypes = pd.read_csv(os.path.join(data_path, "drug-pathway.csv"))
            self.df_disease_phenotypes = pd.read_csv(os.path.join(data_path, "disease-pathway.csv"))
    ...
    def build_dgl_hypergraph(self):
        ...
        phenotype_to_drugs = defaultdict(set)
        ...
        for _, row in self.df_drug_phenotypes.iterrows():
            drug_idx = int(row['drug_idx'])

            if self.dataset != "BioKG":
                phenotype_idx = int(row['phenotype_idx'])
            else:
                phenotype_idx = int(row['pathway_idx'])

            phenotype_to_drugs[phenotype_idx].add(drug_idx)
            
        for _, row in self.df_disease_phenotypes.iterrows():
            disease_idx = int(row['disease_idx'])
            ...
            phenotype_to_diseases[phenotype_idx].add(disease_idx)
        ...
        drug_phenotype_edges = []
        disease_phenotype_edges = []
        phenotype_drug_edges = []
        phenotype_disease_edges = []
        
        for phenotype_idx in range(self.num_phenotypes):
            for drug_idx in phenotype_to_drugs.get(phenotype_idx, []):
                drug_phenotype_edges.append((drug_idx, phenotype_idx))
                phenotype_drug_edges.append((phenotype_idx, drug_idx))
                
            for disease_idx in phenotype_to_diseases.get(phenotype_idx, []):
                disease_phenotype_edges.append((disease_idx, phenotype_idx))
                phenotype_disease_edges.append((phenotype_idx, disease_idx))
        
        graph_data = {
            ('drug', 'drug-gene', 'gene'): drug_gene_edges,
            ('disease', 'disease-gene', 'gene'): disease_gene_edges,
            ('drug', 'drug-phenotype', 'phenotype'): drug_phenotype_edges,
            ('disease', 'disease-phenotype', 'phenotype'): disease_phenotype_edges,
            
            ('gene', 'gene-drug', 'drug'): gene_drug_edges,
            ('gene', 'gene-disease', 'disease'): gene_disease_edges,
            ('phenotype', 'phenotype-drug', 'drug'): phenotype_drug_edges,
            ('phenotype', 'phenotype-disease', 'disease'): phenotype_disease_edges}
        
        self.g = dgl.heterograph(graph_data)
        
        self.g = self.g.to(self.device)
        
        feat = torch.empty(self.num_drugs, self.in_dim).to(self.device)
        init.xavier_uniform_(feat)
        self.g.nodes['drug'].data['feat'] = feat

        feat = torch.empty(self.num_diseases, self.in_dim).to(self.device)
        init.xavier_uniform_(feat)
        self.g.nodes['disease'].data['feat'] = feat

        feat = torch.empty(self.num_genes, self.in_dim).to(self.device)
        init.xavier_uniform_(feat)
        self.g.nodes['gene'].data['feat'] = feat

        feat = torch.empty(self.num_phenotypes, self.in_dim).to(self.device)
        init.xavier_uniform_(feat)
        self.g.nodes['phenotype'].data['feat'] = feat

    def sample_negative_pairs(self):
        num_negatives = int(len(self.drug_disease_pairs) * self.negative_ratio)
    ...
    def build_training_dataset(self):
        negative_pairs = self.sample_negative_pairs()

        all_pairs = self.drug_disease_pairs + negative_pairs
        all_labels = [1] * len(self.drug_disease_pairs) + [0] * len(negative_pairs)
    ...
    def get_stratified_kfold_splits(self, shuffle=True):
        ...
        for train_idx, test_idx in skf.split(self.np_all_pairs, self.np_all_labels):
            ...
            train_pairs, val_pairs, train_labels, val_labels = train_test_split(temp_train_pairs, temp_train_labels,
            test_size = self.validation_ratio, stratify = temp_train_labels, random_state = self.seed + fold_idx)
